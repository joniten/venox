...
const fs = require('fs'); const path = require('path'); const express = require('express'); const cors = require('cors');
const app = express();
app.use(cors()); app.use(express.json());
app.post('/api/order', (req, res) => { try { console.log('ПОЛУЧЕН ЗАКАЗ:', req.body);
const order = {
  ...req.body,
...
app.listen(process.env.PORT || 3000, () => { console.log('Сервер замовлень запущено на порту 3000'); });
const ORDERS_FILE = path.join(__dirname, 'orders.json');
function saveOrder(order) { const orders = JSON.parse( fs.readFileSync(ORDERS_FILE, 'utf8') );
orders.push({ id: Date.now(), ...order, createdAt: new Date().toISOString() });
fs.writeFileSync( ORDERS_FILE, JSON.stringify(orders, null, 2), 'utf8' ); }
const { Telegraf, Markup } = require('telegraf');
const bot = new Telegraf(process.env.BOT_TOKEN);
const ADMIN_ID = 966297755;
const isAdmin = (ctx) => ctx.from?.id === ADMIN_ID;
bot.command('admin', (ctx) => { if (!isAdmin(ctx)) { return ctx.reply('⛔️ Доступ заборонено.'); }
